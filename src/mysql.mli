(* Ocaml-MySQL: MySQL bindings for Ocaml. Copyright (C) 2001-2003 Shawn Wagner
   <shawnw@speakeasy.org> 1998, 1999 Christian Lindig <lindig@eecs.harvard.edu>
...
type db_option =
  ...
  | OPT_READ_TIMEOUT of int
      (** The timeout in seconds for attempts to read from the server. Each
          ...
          the total effective timeout value is three times the option value. *)
  | OPT_WRITE_TIMEOUT of int
      (** The timeout in seconds for attempts to write to the server. Each
          attempt uses this timeout value and there are net_retry_count retries
          if necessary, so the total effective timeout value is net_retry_count
  ...
  | SET_CHARSET_DIR of string
      (** The path name to the directory that contains character set definition
          files. *)
...
(** Initialize library (in particular initializes default character set for
    {!escape} NB it is recommended to always use {!real_escape}) NB init is
    called automatically when db handle is created *)
...
(** [set_charset dbd charset] sets the current character set for [dbd] (aka
    [SET NAMES]). It is strongly recommended to set the charset explicitly after
    connecting to database, using this function. Available character sets are
    stored in [INFORMATION_SCHEMA.CHARACTER_SETS] table ([SHOW CHARACTER SET]). *)

val change_user : dbd -> db -> unit
(** [change_user dbd db] tries to change the current user and database. The host
    and port fields of db are ignored. *)
...
val ping : dbd -> unit
(** [ping dbd] makes sure the connection to the server is up, and re-establishes
    it if needed. *)
...
(** When most of the API functions fail, they raise this exception with a
...
(** [exec dbd str] executes a SQL statement and returns a handle to obtain the
    result. Check [status] for errors! *)

(** {2 Getting the results of a query} *)

val fetch : result -> string option array option
(** [fetch result] returns the next row from a result as [Some a] or [None] in
    case there is no next result. The array [a] contains the values from the
    current row, where NULL values are denoted by [None]. Use [column] to fetch
    single values by field name instead of by position *)
...
(** [iter result f] applies f to each row of result in turn, starting from the
    first. iter_col applies f to the value of the named column in every row.

    The iter versions return unit, the map versions return a list of the results
    of all the function applications. If there were no rows in the result,
    returns an empty list.

    The iter forms are all tail-recursive, so they can be used with any size of
    results. The map forms are tail-recursive, but take up space with the list
    they build. *)
...
(** The type of a database field. Each of these represents one or more MySQL
    data types. *)
type dbty =
  | IntTy
  | FloatTy
...
(** [affected result] returns the number of rows changed by the last UPDATE, or
    deleted by the last DELETE, or added by the last INSERT, or the number of
    rows returned by the last SELECT *)

val insert_id : dbd -> int64
(** [insert_id result] returns the ID generated by the last INSERT query in a
    table with an AUTO_INCREMENT column. See the MySQL documentation for
    caveats. *)
...
(** [escape str] returns the same string as [str] in MySQL syntax with special
    characters quoted to not confuse the MySQL parser.

    @deprecated This function poses a security risk (doesn't take into
    consideration the character set of the current connection and may allow SQL
    injection pass through). Use {!real_escape} instead. *)

val real_escape : dbd -> string -> string
(** [real_escape dbd str] returns [str] encoded to an escaped SQL string
    according to the current character set of [dbd] *)

(** [xxx2ml str] decodes a MySQL value of type xxx into a corresponding OCaml
    value *)
...
(** [opt f v] applies [f] to optional value [v]. Use this to fetch data of known
    type from database fields which might be NULL: [opt int2ml str] *)

val not_null : ('a -> 'b) -> 'a option -> 'b
(** [not_null f v] applies [f] to [Some v]. Use this to fetch data of known type
    from database fields which never can be NULL: [not_null int2ml str] *)

(** [ml2xxx v] encodes [v] into MySQL syntax. [ml2rxxx v] encodes [v] into MySQL
    syntax using [real_escape]. *)

val ml2str : string -> string
(** @deprecated This function uses {!escape} which poses a security risk. Use
    {!ml2rstr} instead. *)
...
(** [values vs] takes a list of strings and returns a string "(a,b,c ..)" where
    values are separated by comma and the whole list is enclosed into
    parentheses. This is useful to construct SQL `insert ... values ( .. )'
    statements *)

(** {1 Prepared statements} *)

(** Prepared statements with parameters. Consult the MySQL manual for detailed
    description and possible problems. *)
module Prepared : sig
  type stmt
  (** Prepared statement *)
  ...
  (** Create prepared statement. Placeholders for parameters are [?] and
      [\@param]. Returned prepared statement is only valid in the context of
      this connection and can be reused many times during the lifetime of the
      connection. *)
  ...
  (** @return the error code for the last operation on this statement, [0] for
      success. *)
