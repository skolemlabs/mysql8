...
(* Do not change any type definition that is used by external functions without
   changing the C source code accordingly! *)
...
let quick_change ?user ?password ?database conn =
  change_user conn
    { defaults with dbuser = user; dbpwd = password; dbname = database }
...
(* [sub start len str] returns integer obtained from substring of length [len]
   from [str] *)

let sub start len str = int_of_string (String.sub str ~pos:start ~len)

(* xxx2ml parses a string returned from a MySQL field typed xxx and turns it
   into a corresponding OCaml value.

   MySQL uses the following representations for date/time values

   DATETIME yyyy-mm-dd hh:mm:ss DATE yyyy-mm-dd' TIME hh:mm:ss' YEAR yyyy'
...
(* [set2ml str] parses a comma separated list of words in [str] and returns them
   in a list. MySQL uses this format for sets of values *)
...
let set2ml str =
  ...
  and loop acc p_start i =
    if i = String.length str then
      String.sub str ~pos:p_start ~len:(String.length str - p_start) :: acc
    else (
      match str.[i] with
      | ' '
      | '\t'
...
(* [column result] returns a function [col] which fetches columns from results
   by column name. [col] has type string -> 'a array -> 'b. Where the first
   argument is the name of the column.

   let r = exec dbd "select * from table" in let col = col r in let rec loop =
   function None -> [] Some a -> not_null int2ml (col "label" a) :: loop (fetch
   r) in loop (fetch r) *)
    ...
    | n ->
      let rec loop i map =
        if i = n then
          ...
          loop (i + 1) (StrMap.add ~key:names.(i) ~data:i map)
      in
      loop 0 StrMap.empty
  in
...
(* ml2xxx encodes OCaml values into strings that match the MysQL syntax of the
   corresponding type *)
...
(* [values vs] creates from a list of values in MySQL format a vector (x,y,z,..)
   for the MySQL values construct *)
